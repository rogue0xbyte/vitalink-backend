import { createReport, createFile } from '../../services/databasepg.mjs';
...
uploadReport.post('/patient/uploadReport', upload.single("file"), authenticateToken, async (req, res) => {
    ...
    try{
        ...
        if (req.session.tokens.includes(sessionToken)) {
            try {
                ...
                const fileName = Date.now().toString().slice(0, -2) + '-' + req.file.originalname; // Assuming the filename is sent in the request
                
                // Construct file path and type
                const filePath = `../../files/${fileName}`;
                const type = req.body.type + "_Report"; // Assuming the file type is sent in the request

                // type:
                //    others => Other Medications
                //    side => Side Effects
                //    prolonged => Recent Illnesses
                ...
                await createReport(req.body.type,
                    currentDatetime, req.body.details, savedFile.id,
                    req.body.startDate, req.body.endDate, req.body.location,
                    req.body.labName, patientId);
