...
async function addDoctor(cat, id, name, password = "password") {
    try {
        ...
        await insertDoctor(cat, id, name, passHash, "/static/images/empty_user.jpg");
...
async function updateINR(PID, level, location, labName, file, datetimeValue = new timezone.Date().toISOString()) {
    try {
        // Connect to the database
        await connectToDatabase();
...
async function listMissedDoses(PID) {
    try {
        ...
        const input_date = strptime(patient["StartDate"], '%d %B \'%y');
        ...
        const start_date = fromisoformat(cal);
        ...
        for (const dosage of doses) {
            if (dosage["DateTime"]) {
                const dosedate = fromisoformat(dosage["DateTime"].replace("I", ""));
